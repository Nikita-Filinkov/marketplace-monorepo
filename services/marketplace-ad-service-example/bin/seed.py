...
async def register(client: httpx.AsyncClient, user: dict) -> None:
    ...
    if resp.status_code == 201:
        ...
        logger.info("registered user %s (user_id=%s)", user["email"], user_id)
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        if total_needed > len(items):
            raise RuntimeError(
                f"need {total_needed} unique items but only {len(items)} are defined "
                f"in {SEED_FILE}; reduce SEED_ADS_PER_USER or add more items"
            )
        shuffled = items.copy()
